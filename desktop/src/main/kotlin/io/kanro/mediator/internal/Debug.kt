...
private val words = Thread.currentThread().contextClassLoader.getResourceAsStream("word.txt").reader().readLines()
...
suspend fun randomCall(): CallTimeline {
    ...
    return CallTimeline().apply {
        this.emit(
            CallEvent.Start(
                "$randomHost:${Random.nextInt(1000, 9999)}",
                randomMethod,
                MethodType.UNKNOWN,
                randomMetadata()
            )
...
suspend fun emitCall(timeline: CallTimeline) {
    ...
    timeline.emit(CallEvent.Input(byteArrayOf()))
    delay(Random.nextLong(1, 30))
    timeline.emit(CallEvent.Accept(randomMetadata()))
    delay(Random.nextLong(1, 30))
    timeline.emit(CallEvent.Output(byteArrayOf()))
    delay(Random.nextLong(1, 30))
    ...
    timeline.emit(
        CallEvent.Close(
            Status.fromCodeValue(Random.nextInt(17))
                .withDescription(randomDescriptor()),
            randomMetadata()
        )
    )
}
